...
function App() {
  ...
  // 출석체크
  const [attendanceOpen, setAttendanceOpen] = useState(false) // 출석체크 모달 열림
  const [attendanceData, setAttendanceData] = useState(null) // { currentDay, claimedDays, weekStartDate }
  const attendanceAutoShownRef = useRef(false) // 자동 팝업 한번만 (ref로 무한루프 방지)
  ...
  useEffect(() => {
    ...
    // 🛡️ 부화장치 보호: incubator 알이 갑자기 모두 사라지면 저장 차단
  ...
  const handleSlotClick = (index) => {
    ...
    // 부화장치가 잠겨있으면 알림 다이얼로그 (수리된 슬롯은 제외)
  ...
  // 부화장치 초기화: 슬롯 3,4를 다시 잠금, 알 삭제, unlockedSlots 초기화
  const handleResetIncubator = async () => {
    if (!session?.user?.id) return
    try {
      // 슬롯 3,4에 알이 있으면 DB에서 삭제
  ...
  function loadAttendanceData(userData) {
    ...
    // 오늘 이미 출석했으면 currentDay를 다음으로
    if (data.lastClaimDate === todayDate && data.currentDay < 7) {
      // 이미 out석한 상태 — 그대로 유지
    }
    ...
    <div className={`app ${tab === 'egg' ? 'app--bg-egg' : ''} ${tab === 'field' ? 'app--bg-field' : ''} ${tab === 'sanctuary' ? 'app--bg-sanctuary' : ''}`}>
      ...
      <div className="app-frame">
        ...
        <SettingsPanel
          ...
          onResetIncubator={handleResetIncubator}
        ...
        <AttendanceCheck
          ...
          onClaimReward={handleAttendanceClaim}
